...
<html>
    ...
    <body>
        ...
        <div id="content">
                <?php
                $bkt -> test_listen('SELECT * FROM hv INNER JOIN class
                on hv.ID_Lop = class.ID_lop INNER join listening
                on class.ID_listen = listening.ID_listen
                WHERE hv.SDT = '.$sdt.'');
                ?>
                ...
                if (isset($_REQUEST['submit']))
                {
                    switch($_REQUEST['submit'])
                    {
                        case 'Nộp Bài':
                            ...
                            $answer = rtrim($tl, '/');
                            $id_listen = $_SESSION['id_listen'];
                            $mydate=getdate();
                            $date= "$mydate[year]-$mydate[month]-$mydate[mday]";
                            if(($bkt->add("INSERT INTO `ketqua_listen` (`sdt_hv`, `id_listen`, `ketqua`, `ngaykiemtra`) VALUES ('$sdt', '$id_listen', '$answer', '$date')"))==1)
                            {
