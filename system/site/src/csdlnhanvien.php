...
class csdl
{
    ...
    function timkiem($sql)
    {
        ...
        if($i>0)
        {
            while($row=mysqli_fetch_array($ketqua))
            {
                ...
                <center><button><a href="NV_capnhattaikhoan.php?id='.$sdt.'&&act='.$act.'">Change</a></button></center>
                </form>
                ';
            }
        }
    ...
    function xuatdstaikhoan($sql){
        ...
        if($i>0)
        {
            while($row=mysqli_fetch_array($ketqua))
            {
                ...
                echo'
                    <tr>
                    <td><a href="NV_capnhattaikhoan.php?id='.$sdt.'&&act='.$act.'">'.$sdt.'</a></td>
                    <td>'.$name.'</td>
                    <td>'.$act1.'</td>
    ...
    function xuatcomboboxgv($sql)
	{
		...
		if($i>0)
		{
			echo '<select name="sdt_gv" id="quyen" style="float:right;width:160px;" required>';
			while($row= mysqli_fetch_array($ketqua))
			{
				$sdt=$row['SDT'];
				$ten=$row['Name'];
		...
		if($i>0)
		{
            while($row= mysqli_fetch_array($ketqua))
            {
                ...
                <div id="lophoc">
							...
							<div id="ifo"><a href="../NV/NV_info-class.php?idlop='.$ID.'"><button>more-inf</button></a></div>
				</div>
                ';
            }
        }
